...
A self-modifying agentic harness. The agent's loop, its tools, and the chat
...
narrow, mediated slices of them through the contract in [`wit/genesis.wit`](wit/genesis.wit).
...
```bash
OPENROUTER_API_KEY=sk-... cargo run --release -p genesis
```
...
```bash
OPENROUTER_API_KEY=test OPENROUTER_BASE_URL=http://127.0.0.1:7788 GENESIS_MODEL=mock/echo cargo run --release -p genesis
```
...
**Models.** A per-conversation override, chosen from `GENESIS_MODELS`. Empty
...
| `restart_orchestrator` | Replaces the Genesis process, for changes to the native binary or to startup-only settings |
...
crash, or anything else — is picked up again when Genesis comes back. Resuming
...
- **Circuit breaker.** Repeated traps from one slot roll it back to its last
...
- **Revisions and snapshots.** Every activation freezes an immutable revision —
  ...
  every slot was running. Rollback restores source and binary together, so the
...
- **`/admin`.** A control panel rendered by the orchestrator itself, with no
  WebAssembly in its path. It lists every slot's history and every system
  snapshot with one-click restore, and keeps working when every guest is broken.

Writes are constrained too: paths cannot escape a slot's source tree, and
...
Genesis reads `genesis.toml` from the project root. Every setting is optional —
...
2. `genesis.toml`, or whatever `GENESIS_CONFIG` points at
...
| Vendor | Behaviour | What Genesis does |
|---|---|---|
...
Genesis therefore places up to four breakpoints — the limit Anthropic allows —
...
A key in the file is a key on disk. `.gitignore` excludes `genesis.local.toml`
and `*.local.toml`, so if this repo is ever version controlled, keep the real key
in one of those and point `GENESIS_CONFIG` at it. Genesis holds the key in a type
...
| `GENESIS_CONFIG` | which config file to read |
| `GENESIS_ROOT` | the project root |
| `GENESIS_BIND` | `server.bind` |
| `GENESIS_MODEL` | `llm.model` |
| `GENESIS_MODELS` | the model picker, as `id=Label` pairs |
| `GENESIS_DEFAULT_MODE` | `agent.default_mode` |
| `GENESIS_SYSTEM_PROMPT` | the system prompt |
| `GENESIS_SANDBOX` | `sandbox.enabled` |
| `GENESIS_DEVKIT` | `devkit.enabled` |
| `GENESIS_LOG` | tracing filter |

Budget and limit values have `GENESIS_`-prefixed overrides too, named after
their keys — `GENESIS_TURN_BUDGET_SECS`, `GENESIS_MAX_ITERATIONS`, and so on.

## Layout

```
genesis.toml             configuration; every path below is one of its settings
wit/genesis.wit          the host/guest contract — changing it rebuilds every guest
crates/genesis           the kernel: loader, pipeline, revisions, watchdogs, web
...
data/genesis.redb        sessions, event log, revision registry
```
...
watcher's path-to-slot mapping.

## Not yet built

- **Docker exec sandbox.** The capability is defined and wired, but the
  implementation is a stub; with `GENESIS_SANDBOX=false` the agent is simply not

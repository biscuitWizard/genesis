...
/// (slot key, revision) -> RevisionRow (json)
...
impl Store {
    ...
    pub fn reconcile_interrupted_turns(&self, note: &str) -> Result<Vec<Interrupted>> {
            ...
            for call in unanswered_tool_calls(&events) {
                self.append_event(
                    ...
                    SessionEvent::ToolResult(ToolOutcome {
                        ...
                        name: call.name,
                        ok: false,
                        content: "Interrupted: Genesis restarted before this finished.                                   Run it again if the result still matters."
                            .to_string(),
                    }),
                )?;
            }
...
impl Store {
    /// Next unused revision number for a slot. Revisions never restart, even
    /// after a rollback, so history is always append-only.
    pub fn next_revision(&self, slot_key: &str) -> Result<u64> {
        let rows: Vec<crate::revisions::RevisionRow> = self.list_revisions(slot_key)?;
        Ok(rows.last().map(|r| r.revision).unwrap_or(0) + 1)
    }

    pub fn put_revision<T: serde::Serialize>(&self, slot_key: &str, revision: u64, row: &T) -> Result<()> {
        let txn = self.db.begin_write()?;
        {
            let mut t = txn.open_table(REVISIONS)?;
            t.insert((slot_key, revision), serde_json::to_vec(row)?.as_slice())?;
        }
        txn.commit()?;
        Ok(())
    }
    ...
    pub fn get_revision<T: serde::de::DeserializeOwned>(
        ...
        match t.get((slot_key, revision))? {
            Some(v) => Ok(Some(serde_json::from_slice(v.value())?)),
            None => Ok(None),
        }
    }

    pub fn list_revisions<T: serde::de::DeserializeOwned>(&self, slot_key: &str) -> Result<Vec<T>> {
        let txn = self.db.begin_read()?;
        let t = txn.open_table(REVISIONS)?;
        let mut out = Vec::new();
        for row in t.range((slot_key, 0u64)..=(slot_key, u64::MAX))? {
            let (_, v) = row?;
            out.push(serde_json::from_slice(v.value())?);
        }
        Ok(out)
...
mod tests {
    ...
    fn session_mode_and_model_round_trip() {
        ...
        // Clearing the override falls back to the harness default.

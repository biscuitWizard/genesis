...
//!   before it replaces anything. Genesis refuses to start on a bad config, so
...
pub fn set(cfg: &Config, key: &str, value: &str) -> Result<String> {
    ...
    // The whole point of the guard: a config Genesis cannot load leaves it
    ...
    Ok(format!(
        "{key}: {} -> {} (written to {}). Configuration is read at startup, so \
         restart Genesis for this to take effect.",
...
"#;
    ...
    fn comments_survive_an_edit() {
        ...
        assert!(text.contains("# Genesis configuration."));
    ...
    fn a_change_that_would_not_load_is_refused() {
        ...
        // A bind address that cannot parse would stop Genesis starting at all.

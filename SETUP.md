...
Genesis is one native binary (the orchestrator) plus a handful of WebAssembly
...
*component*, which is why Genesis needs no `cargo-component` or similar tooling.
...
You also need a C linker and OpenSSL-free TLS is already handled (Genesis uses
...
You should see `wasm32-wasip2` listed. Genesis was developed against Rust 1.95.

**An OpenRouter API key** from <https://openrouter.ai/keys>. Genesis talks to
...
The file `genesis.toml` in the project root is read on startup. Every setting
in it is optional — delete the file entirely and Genesis still runs on its
built-in defaults.
...
```bash
cp genesis.toml genesis.local.toml
```

Edit `genesis.local.toml` and set, under `[llm]`:

```toml
api_key = "sk-or-v1-..."
```

Then point Genesis at it:

```bash
# Linux
GENESIS_CONFIG=genesis.local.toml cargo run --release
```

```powershell
# Windows PowerShell
$env:GENESIS_CONFIG = "genesis.local.toml"; cargo run --release
```

`.gitignore` already excludes `genesis.local.toml` and `*.local.toml`. It does
**not** exclude `genesis.toml`, so a key placed there would be committed.
</details>

Genesis holds the key in a type whose `Debug` prints `Secret(***)`, so it will
...
```powershell
# Windows
.\target\release\genesis.exe
```

On the first start you will see it build each guest component:

```
INFO starting genesis root=/path/to/genesis
INFO building slot=gateway/web
INFO loaded slot=gateway/web revision=1 took_ms=...
INFO building slot=agent
INFO loaded slot=agent revision=1 took_ms=...
INFO hot reload active
INFO genesis listening addr=127.0.0.1:7777
```
...
And Genesis in another, pointed at it:

```bash
# Linux
OPENROUTER_API_KEY=test \
OPENROUTER_BASE_URL=http://127.0.0.1:7788 \
GENESIS_MODEL=mock/echo \
cargo run --release
```
...
```powershell
# Windows
Remove-Item Env:OPENROUTER_BASE_URL, Env:GENESIS_MODEL -ErrorAction SilentlyContinue
```

```bash
# Linux
unset OPENROUTER_BASE_URL GENESIS_MODEL
```
...
Genesis behaves identically on both platforms except for these:
...
`genesis.toml` to use `bash`, `pwsh`, `zsh`, or anything else.
...
you can — if that is not what you want, turn it off in `genesis.toml`:
...
**Genesis refuses to start naming a config key**
...
Another copy is running. Genesis retries the bind for 15 seconds to allow for
restarts, then gives up. Find and stop it:

```powershell
# Windows
Get-Process genesis | Stop-Process -Force
```
...
Or change `server.bind` in `genesis.toml`.

**`slot failed to start slot=tool/delete-tool`**
...
built component. Deleting both resets Genesis to a clean install. Stop it first.
...
```
genesis.toml             configuration; every setting documented inline
wit/genesis.wit          the host/guest contract — editing it rebuilds every guest
crates/genesis           the orchestrator
...
data/genesis.redb        conversations, event log, revision registry
```

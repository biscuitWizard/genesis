...
//! config. Put it in `genesis.local.toml`, which is gitignored, rather than in
//! `genesis.toml`, which is committed.
...
fn api_key(config: &Value) -> Result<&str, String> {
    config
        ...
        .ok_or_else(|| {
            "no Exa API key configured. Add this to genesis.local.toml, which is gitignored:\n\
             \n\
             [tools.web-summarize]\n\
